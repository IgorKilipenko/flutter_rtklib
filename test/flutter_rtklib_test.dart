...
void print_debug(ffi.Pointer<ffi.Char> str, int len) {
...
void main() {
  ...
  testing.group("Bindings tests", () {
    ffi.Pointer<struct_sizes_t>? sizesPtr;
    testing.setUp(() {
      final printCallback =
          ffi.Pointer.fromFunction<_wrappedPrint_C>(print_debug);
      dylibRtklib.flutter_initialize(printCallback);
      sizesPtr = dylibRtklib.getStructSizes();
    });

    testing.tearDown(() {
      if (sizesPtr != null && sizesPtr!.address != 0) {
        pkg_ffi.calloc.free(sizesPtr!);
      }
    });

    testing.test("Is RtkLib", () {
      testing.expect(dylibRtklib, testing.isA<RtkLib>());
    });
