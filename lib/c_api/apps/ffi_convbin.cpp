...
const int _debug_level = 3;
const int _error_level = 1;
                   ...
                   char *dir)
{
    ...
    flutter_trace(_debug_level,"input file  : %s (%s)\n",ifile,formatstrs[format]);
    
    if (*ofile[0]) flutter_trace(_debug_level,"->rinex obs : %s\n",ofile[0]);
    if (*ofile[1]) flutter_trace(_debug_level,"->rinex nav : %s\n",ofile[1]);
    if (*ofile[2]) flutter_trace(_debug_level,"->rinex gnav: %s\n",ofile[2]);
    if (*ofile[3]) flutter_trace(_debug_level,"->rinex hnav: %s\n",ofile[3]);
    if (*ofile[4]) flutter_trace(_debug_level,"->rinex qnav: %s\n",ofile[4]);
    if (*ofile[5]) flutter_trace(_debug_level,"->rinex lnav: %s\n",ofile[5]);
    if (*ofile[6]) flutter_trace(_debug_level,"->rinex cnav: %s\n",ofile[6]);
    if (*ofile[7]) flutter_trace(_debug_level,"->rinex inav: %s\n",ofile[7]);
    if (*ofile[8]) flutter_trace(_debug_level,"->sbas log  : %s\n",ofile[8]);
    
    if (!convrnx(format,opt,ifile,ofile)) {
        flutter_trace(_debug_level,"\n");
        return -1;
    }
    flutter_trace(_debug_level,"\n");
    return 0;
...
static int __main(int argc, char **argv)
{
    ...
    if (!*ifile) {
        flutter_trace(_debug_level,"no input file\n");
        return -1;
    }
    if (format<0) {
        flutter_trace(_debug_level,"input format can not be recognized\n");
        return -1;
    }
...
extern int convbin_convert_cmd(int argc, char **argv, rnxopt_t *opt, int trace) {
    ...
    if (format<0) {
        flutter_trace(_error_level,"input format can not be recognized\n");
        return -1;
    }
    sprintf(opt->prog,"%s %s %s",PRGNAME,VER_RTKLIB,PATCH_LEVEL);
    sprintf(opt->comment[0],"log: %-55.55s",ifile);
